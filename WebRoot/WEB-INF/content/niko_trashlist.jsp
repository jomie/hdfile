  ...
  <script type="text/javascript">
  	$(function(){
  		...
  		$("#renameBtn").bind("click", function(){
  			...
  			$.post("rename-bxx", params, function(data){
  				...
  				if(resultObj.status == "success")
  					window.location =  "filelist-trash?dir=" + $("#dir").val();
  				else
  					alert("重命名失败.");
  			});
  			//alert("move-bxx?dir=" + $("#dir").val() +"&oldName="+$("#oldName").val()+"&newDir="+$("#newDir").val());
  		...
  		$("#deleteBtn").bind("click", function(){
  			var jump = "rmdelete-trash?dir="+"<s:property value='dir'/>"+"&filename="+$("#deleteName").val();
  					//+ "&fileId="+$("#deleteId").val();
  			//alert(jump);
  			location = jump;
  		});
...
<body style="padding:0px;">
  ...
  <div class="" style="margin:70px 0px 0px 0px">
   	<jsp:include page="left_bar_trash.jsp"></jsp:include>
     
     
   <div class="row-fluid">
     
	 <!-- 10个 -->
     <div class="span10 offset2"  style="margin-right:50px;">
       

      <ul class="breadcrumb">
        <li><a href="filelist-trash?dir=/">根目录</a></li>
        ...
        <s:iterator value="folderLevelArr" id="oneFolder">
        	<li><a href="filelist-trash?dir=<s:property value='#tempDirLvl + #oneFolder + \"/\"'/>"><s:property value="#oneFolder"/> </a><span class="divider">/</span></li>
	...
	<script type="text/javascript">
	  ...
	  <div class="pagination" style="">
		      <ul>
		        <li>
		        <a reclass="btn" href="filelist-trash?dir=<s:property value='dir'/>&pageNo=<s:property value='lastPageNo'/>">上一页</a>
				</li>
				<li>
				<a reclass="btn" href="filelist-trash?dir=<s:property value='dir'/>&pageNo=<s:property value='nextPageNo'/>">下一页</a>
				</li>
				<li class="disabled">
      ...
      <table class="table table-hover">
       ...
       <s:iterator value="list1" id="m">
    		...
    		<td id="td-2nd">
    			<s:if test="#d==\"/\"">
	    			<a href="filelist-trash?dir=/<s:property value='#m["pathSuffix"]'/>/"><s:property value="#m['pathSuffix']"/></a>
    			</s:if>
    			<s:else>
	    			<a href="filelist-trash?dir=<s:property value='#d'/><s:property value='#m["pathSuffix"]'/>/"><s:property value="#m['pathSuffix']"/></a>
    			</s:else>
    		</td>
    		<td>&nbsp;<a><s:property value="#m['length']"/></a></td>
    		<!-- 下载_进入 
    		<td>
    		    <a class="btn btn-info" href="filelist-trash?dir=/<s:property value='#m["pathSuffix"]'/>/">进入</a>
    		</td>
    		<td><a name="moveShowModalBtn" href="#moveModal" role="button" class="btn btn-info" data-toggle="modal"
    						oldName="<s:property value='#m["pathSuffix"]'/>">移动至</a>
    		</td>
    		<td><a name="renameShowModalBtn" href="#renameModal" role="button" class="btn btn-info" data-toggle="modal"
    					oldName="<s:property value='#m["pathSuffix"]'/>">重命名</a></td>
    		-->
    		<td><a name="" href="revert?dir=<s:property value='dir'/>&oldName='<s:property value='#m["pathSuffix"]'/>'" class="btn btn-info">恢复该文件</a></td> 
    		<td><a name="deleteShowModalBtn" href="#deleteModal" role="button" class="btn btn-danger" data-toggle="modal" 
    						deleteName="<s:property value='#m["pathSuffix"]'/>">彻底删除</a></td> 
    		</tr>
    	</s:iterator>
    ...
    <div class="pagination">
      <ul>
        <li>
        <a class="btn" href="filelist-trash?dir=<s:property value='dir'/>&pageNo=<s:property value='lastPageNo'/>">上一页</a>
		</li>
		<li>
		<a class="btn" href="filelist-trash?dir=<s:property value='dir'/>&pageNo=<s:property value='nextPageNo'/>">下一页</a>
		</li>
		<li>
